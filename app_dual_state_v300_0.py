# ==========================================================
# 💎 Celestial Titan God AI v300.0 — Final Hybrid Core Evolution
# ==========================================================
...
# THEME COLORS
if theme=="Cosmic":
    bg="#000014"; fg="#b5faff"; accent="#00ffff"
elif theme=="Dark Crystal":
    bg="#080808"; fg="#e6e6e6"; accent="#9b59b6"
else:
    bg="#000000"; fg="#00ffea"; accent="#ff007f"
...
c1,c2=st.columns(2)
with c1: game=st.selectbox("Game",["GA Pick-3","FL Pick-4"])
with c2: draw=st.radio("Draw Type",["Midday","Evening"],horizontal=True)
...
# 🧾 RESULT ENTRY
# ==========================================================
st.write("---"); st.subheader("📥 Log Official Result")
c1,c2,c3,c4=st.columns(4)
with c1: rg=st.selectbox("Game",["GA Pick-3","FL Pick-4"],key="resg")
with c2: rd=st.radio("Draw Type",["Midday","Evening"],horizontal=True,key="resd")
with c3: rdate=st.date_input("Date",datetime.date.today())
with c4: rnum=st.text_input("Winning Number","")
...
if st.button("💾 Save Result"):
    ...
    if rec["hit"]: st.success(f"🎯 Titan Hit Detected — {rnum} ({rg} {rd})")
    else: st.warning(f"Saved {rnum} ({rg} {rd}) — no hit match.")

# ==========================================================
# 📊 ACCURACY + HISTORY
# ==========================================================
st.write("---"); st.subheader("📊 Accuracy Dashboard")
chart=accuracy_chart()
if chart:
    fig,acc=chart; st.pyplot(fig); st.metric("Current Accuracy",f"{acc}%")
else:
    st.info("No logged results yet — Titan calibration pending.")

st.write("---"); st.subheader("📖 Forecast History")
