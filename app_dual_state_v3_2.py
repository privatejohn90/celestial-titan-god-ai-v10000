# ==============================================================
# 💎 Celestial Titan Dual-State Forecast Lab v3.2 — Final Stable Build
# 🌌 GA Pick-3 + FL Pick-4 — Unified Forecast Console + Memory Engine
# ==============================================================

import streamlit as st, json, os, random, datetime, pandas as pd, time
...
def compute_accuracy():
    ...
    hits = misses = 0
    for r in results:
        if any(r["result"] in a.get("hit","") for a in acc.get("records", [])):
            hits += 1
        else:
            misses += 1
    total = hits + misses
...
<style>
@keyframes pulseGlow {
  0% { text-shadow: 0 0 10px #00ffff, 0 0 20px #0088ff; }
  50% { text-shadow: 0 0 20px #00ffff, 0 0 40px #00ccff; }
  100% { text-shadow: 0 0 10px #00ffff, 0 0 20px #0088ff; }
}
h1, p { animation: pulseGlow 2.5s infinite alternate; }
</style>
<div style='text-align:center;padding:15px;background:linear-gradient(90deg,#0d1b2a,#1b263b,#0f3460);
color:#9ef0ff;border-radius:15px;box-shadow:0 0 25px #00f0ff80;'>
...
# --------------------------------------------------------------
st.divider()
st.subheader("🗓️ Titan Calendar Sync View")
...
if data:
    ...
    # group by game then by month
    for state_group in df_sorted["game"].unique():
        st.markdown(f"#### 🌠 {state_group}")
        filtered = df_sorted[df_sorted["game"] == state_group]
        for month, subset in filtered.groupby(filtered["date"].dt.strftime("%B %Y")):
            st.markdown(f"<h5 style='color:#9ef0ff;'>{month}</h5>", unsafe_allow_html=True)
            for date, s in subset.groupby(subset["date"].dt.strftime("%Y-%m-%d")):
                numbers = ", ".join(s["result"].astype(str))
...
st_autorefresh = st.experimental_rerun if random.random() < 0.0001 else None
