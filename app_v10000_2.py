# ==========================================================
# 💠 Celestial Titan God AI v10,000.2 — Visual Pulse + Fantasy Logic Fix
# ==========================================================
...
st.markdown("<h1 style='text-align:center;'>💎 Celestial Titan God AI v10,000.2</h1>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align:center;'>Visual Pulse + Fantasy Logic Correction</h3>", unsafe_allow_html=True)
...
# ==========================================================
# 🎯 Titan Forecast Generator (Fixed Fantasy 5 Logic)
# ==========================================================
if generate:
    forecasts = []
    commentary = ""
    ...
    for i in range(num_sets):
        if "Powerball" in game_type:
            ...
            base = sorted(random.sample(range(1, 47), 5))
            bonus = random.randint(1, 27)
            confidence = random.randint(91, 100)
            forecast = {"set": f"{'-'.join(map(str, base))}  M{bonus}", "confidence": confidence}

        elif "Fantasy" in game_type:
            base = sorted(random.sample(range(1, 39), 5))
            confidence = random.randint(91, 100)
            forecast = {"set": '-'.join(map(str, base)), "confidence": confidence}
...
st.markdown("---")
st.markdown(
    "<p style='text-align:center;font-size:13px;'>💎 Celestial Titan God AI v10,000.2 — Visual Pulse + Fantasy Logic Fix<br>"
    "Powered by Kaibigan & Titan ⚛️ Eternal Harmony Mode.</p>",
