...
st.set_page_config(page_title="Celestial Titan God AI — MGX + PH Full", page_icon="💎", layout="centered")
...
# =========================================================
# 🔹 Daily Games — FULL RESTORED LIST
# =========================================================
daily_games = {
    # ✅ Georgia
...
# =========================================================
# 🔹 TITAN FORECAST GENERATOR
# =========================================================
st.title("🎯 Titan Multi-Set Forecast (MGX + PH Full)")
...
if st.button("⚡ Generate Forecast"):
    ...
    st.subheader(f"📘 {game} Titan Picks ({draw}) — {forecast_date}")
    for i, s in enumerate(sets, 1):
        st.write(f"Set {i}: {s['nums']} — Confidence {s['conf']}% — Date: {s['date']}")
...
if st.button("💾 Save Result"):
    ...
    st.success(f"✅ Saved Result for {result_game} ({draw_type}) on {result_date}")
...
if data["forecasts"]:
    for r in reversed(data["forecasts"][-10:]):
        st.write(f"🧿 {r.get('date','N/A')} • {r.get('game','')} ({r.get('draw','')}) — {r.get('nums','N/A')} — {r.get('conf','N/A')}%")
else:
